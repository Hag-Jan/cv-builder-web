import React from "react";
import type {
    ResumeV2 as Resume,
    ContactSectionV2 as ContactSection,
    ExperienceSectionV2 as ExperienceSection,
    EducationSectionV2 as EducationSection,
    SkillsSectionV2 as SkillsSection,
    ProjectsSectionV2 as ProjectsSection,
    CustomSectionV2 as CustomSection,
    SummarySection,
} from "@/types/resume-schema-v2";
import { formatDate } from "@/lib/utils/date-formatter";
import { EntryBlock } from "../preview/EntryBlock";
import { parseSkillLevel } from "@/lib/utils/skill-parser";

/**
 * EXAMPLE DATA FOR PREVIEW/TESTING (Realistic, no lorem ipsum):
 * Name: "Sarah Jenkins"
 * Job Title: "Chief Operating Officer"
 * Contact: "sarah.jenkins@email.com | (555) 123-4567 | New York, NY | linkedin.com/in/sarahjenkins"
 * Summary: "Operations executive with 15+ years of experience scaling tech startups from Series A to acquisition. Proven track record of improving operational efficiency by 40% and managing cross-functional teams of 200+ employees."
 * Experience: "VP of Operations at TechCorp (2018-Present) - Led global expansion into EMEA market, increasing ARR by $15M."
 * Education: "MBA from Harvard Business School (2010)"
 * Skills: "Strategic Planning, P&L Management, M&A, Change Management, Agile Leadership"
 */

const DEFAULT_ACCENT = "#3730A3"; // Deep Indigo
const DEFAULT_FONT = "Georgia, serif";

interface ExecutiveClassicProps {
    resume: Resume;
}

function SectionHeading({ label, accent, color }: { label: string; accent: string; color: string }) {
    return (
        <h2
            className="text-sm font-bold uppercase pb-1 mb-4 border-b-2"
            style={{ color: color, borderBottomColor: accent }}
        >
            {label}
        </h2>
    );
}

export function renderExecutiveClassicBlocks(resume: Resume): React.ReactNode[] {
    const DESIGN = resume.design || {};
    const ACCENT = DESIGN.accentColor || DEFAULT_ACCENT;
    const nameColor = DESIGN.applyColorToName ? ACCENT : "#111827";
    const headingColor = DESIGN.applyColorToHeadings ? ACCENT : "#111827";
    const FONT = DESIGN.fontFamily || DEFAULT_FONT;

    const contact = resume.sections.find((s) => s.type === "contact") as ContactSection | undefined;
    const summary = resume.sections.find((s) => s.type === "summary") as SummarySection | undefined;
    const sorted = [...resume.sections].sort((a, b) => a.order - b.order);

    const blocks: React.ReactNode[] = [];

    // Header Area
    if (contact) {
        blocks.push(
            <EntryBlock key="contact" type="contact" id="contact">
                <div style={{ fontFamily: FONT }} className="mb-6">
                    <h1
                        className="text-4xl font-black mb-2 leading-tight"
                        style={{ color: nameColor }}
                    >
                        {contact.name}
                    </h1>

                    <div className="h-1.5 w-full mb-3" style={{ backgroundColor: ACCENT }}></div>

                    <div className="flex flex-wrap items-center text-[10.5px] font-medium text-gray-700 gap-x-4 gap-y-1">
                        {contact.email && <span>{contact.email}</span>}
                        {contact.email && contact.phone && <span className="text-gray-400">|</span>}
                        {contact.phone && <span>{contact.phone}</span>}
                        {contact.phone && contact.location && <span className="text-gray-400">|</span>}
                        {contact.location && <span>{contact.location}</span>}
                        {contact.location && contact.linkedin && <span className="text-gray-400">|</span>}
                        {contact.linkedin && <span>{contact.linkedin.replace(/^https?:\/\/(www\.)?/, '')}</span>}
                    </div>
                </div>
            </EntryBlock>
        );
    }

    // Summary Area
    if (summary && summary.content) {
        blocks.push(
            <EntryBlock key="summary" type="summary" id="summary">
                <div className="mb-6" style={{ fontFamily: FONT }}>
                    <SectionHeading label="Executive Summary" accent={ACCENT} color={headingColor} />
                    <p className="text-[11.5px] leading-relaxed text-gray-800 text-justify">
                        {summary.content}
                    </p>
                </div>
            </EntryBlock>
        );
    }

    // Processing other sections
    sorted.forEach((section) => {
        if (section.type === "contact" || section.type === "summary") return;

        // Experience
        if (section.type === "experience" && (section as ExperienceSection).items.length > 0) {
            blocks.push(
                <EntryBlock
                    key={`${section.id}-header`}
                    type="header"
                    id={`${section.id}-header`}
                    sectionId={section.id}
                    sectionTitle="Professional Experience"
                >
                    <div style={{ fontFamily: FONT }} className="mt-4">
                        <SectionHeading label="Professional Experience" accent={ACCENT} color={headingColor} />
                    </div>
                </EntryBlock>
            );

            (section as ExperienceSection).items.forEach((item) => {
                blocks.push(
                    <EntryBlock key={item.id} type="entry" id={item.id} sectionId={section.id} sectionTitle="Professional Experience">
                        <div style={{ fontFamily: FONT, marginBottom: "var(--entry-space)" }} className="mb-5 last:mb-6">
                            <div className="flex justify-between items-baseline mb-0.5">
                                <h3 className="text-[13px] font-bold text-gray-900">{item.role}</h3>
                                <time className="text-[11px] font-bold text-gray-600 shrink-0">
                                    {formatDate(item.startDate)} — {formatDate(item.endDate || "Present")}
                                </time>
                            </div>
                            <div className="flex justify-between items-baseline mb-2">
                                <p className="text-[12px] font-semibold text-gray-700 italic">{item.company}</p>
                                {item.location && (
                                    <p className="text-[11px] text-gray-500 italic shrink-0">{item.location}</p>
                                )}
                            </div>
                            {item.bullets && item.bullets.length > 0 && (
                                <ul className="list-disc pl-5 space-y-1">
                                    {item.bullets.map((b, i) => (
                                        <li key={i} className="text-[11.5px] text-gray-800 leading-relaxed pl-1">
                                            {b}
                                        </li>
                                    ))}
                                </ul>
                            )}
                        </div>
                    </EntryBlock>
                );
            });
        }

        // Education
        if (section.type === "education" && (section as EducationSection).items.length > 0) {
            blocks.push(
                <EntryBlock
                    key={`${section.id}-header`}
                    type="header"
                    id={`${section.id}-header`}
                    sectionId={section.id}
                    sectionTitle="Education"
                >
                    <div style={{ fontFamily: FONT }} className="mt-4">
                        <SectionHeading label="Education" accent={ACCENT} color={headingColor} />
                    </div>
                </EntryBlock>
            );

            (section as EducationSection).items.forEach((item) => {
                blocks.push(
                    <EntryBlock key={item.id} type="entry" id={item.id} sectionId={section.id} sectionTitle="Education">
                        <div style={{ fontFamily: FONT, marginBottom: "var(--entry-space)" }} className="mb-4 last:mb-6">
                            <div className="flex justify-between items-baseline mb-0.5">
                                <h3 className="text-[13px] font-bold text-gray-900">{item.school}</h3>
                                <time className="text-[11px] font-bold text-gray-600 shrink-0">
                                    {formatDate(item.date)}
                                </time>
                            </div>
                            <div className="flex justify-between items-baseline">
                                <p className="text-[11.5px] text-gray-800">{item.degree}</p>
                                {item.gpa && (
                                    <p className="text-[11px] font-medium text-gray-600">GPA: {item.gpa}</p>
                                )}
                            </div>
                        </div>
                    </EntryBlock>
                );
            });
        }

        // Skills
        if (section.type === "skills" && (section as SkillsSection).categories.length > 0) {
            blocks.push(
                <EntryBlock
                    key={`${section.id}-header`}
                    type="header"
                    id={`${section.id}-header`}
                    sectionId={section.id}
                    sectionTitle="Core Competencies"
                >
                    <div style={{ fontFamily: FONT }} className="mt-4">
                        <SectionHeading label="Core Competencies" accent={ACCENT} color={headingColor} />
                    </div>
                </EntryBlock>
            );

            const skillsSection = section as SkillsSection;
            blocks.push(
                <EntryBlock key={skillsSection.id} type="skills" id={skillsSection.id} sectionId={section.id} sectionTitle="Core Competencies">
                    <div style={{ fontFamily: FONT, marginBottom: "var(--entry-space)" }} className="mb-6">
                        {skillsSection.categories.map((cat, i) => {
                            const cleanedSkills = cat.skills.map(s => s.trim()).filter(Boolean);
                            if (cleanedSkills.length === 0) return null;

                            return (
                                <div key={i} className="mb-1.5 flex flex-wrap items-start">
                                    <span className="text-[11.5px] font-bold text-gray-900 w-32 shrink-0">{cat.label}:</span>
                                    <span className="text-[11.5px] text-gray-800 flex-1">
                                        {cleanedSkills.map(s => parseSkillLevel(s).name).join(", ")}
                                    </span>
                                </div>
                            );
                        })}
                    </div>
                </EntryBlock>
            );
        }

        // Projects
        if (section.type === "projects" && (section as ProjectsSection).items.length > 0) {
            blocks.push(
                <EntryBlock
                    key={`${section.id}-header`}
                    type="header"
                    id={`${section.id}-header`}
                    sectionId={section.id}
                    sectionTitle="Select Projects"
                >
                    <div style={{ fontFamily: FONT }} className="mt-4">
                        <SectionHeading label="Select Projects" accent={ACCENT} color={headingColor} />
                    </div>
                </EntryBlock>
            );

            (section as ProjectsSection).items.forEach((item) => {
                blocks.push(
                    <EntryBlock key={item.id} type="projects" id={item.id} sectionId={section.id} sectionTitle="Select Projects">
                        <div style={{ fontFamily: FONT, marginBottom: "var(--entry-space)" }} className="mb-5 last:mb-6">
                            <div className="flex justify-between items-baseline mb-0.5">
                                <h3 className="text-[13px] font-bold text-gray-900">
                                    {item.name}
                                    {item.link && (
                                        <a href={item.link} className="font-normal text-[10.5px] italic ml-2 text-blue-600 underline">
                                            {item.link.replace(/^https?:\/\/(www\.)?/, "")}
                                        </a>
                                    )}
                                </h3>
                            </div>
                            {item.techStack && item.techStack.length > 0 && (
                                <p className="text-[10.5px] font-medium text-gray-600 mb-1.5">
                                    {item.techStack.join(", ")}
                                </p>
                            )}
                            {item.description && (
                                <p className="text-[11.5px] text-gray-800 leading-relaxed mb-1.5">
                                    {item.description}
                                </p>
                            )}
                            {item.bullets && item.bullets.length > 0 && (
                                <ul className="list-disc pl-5 space-y-1">
                                    {item.bullets.map((b, i) => (
                                        <li key={i} className="text-[11.5px] text-gray-800 leading-relaxed pl-1">
                                            {b}
                                        </li>
                                    ))}
                                </ul>
                            )}
                        </div>
                    </EntryBlock>
                );
            });
        }

        // Custom
        if (section.type === "custom") {
            const customSection = section as CustomSection;
            blocks.push(
                <EntryBlock
                    key={`${section.id}-header`}
                    type="header"
                    id={`${section.id}-header`}
                    sectionId={section.id}
                    sectionTitle={customSection.title}
                >
                    <div style={{ fontFamily: FONT }} className="mt-4">
                        <SectionHeading label={customSection.title} accent={ACCENT} color={headingColor} />
                    </div>
                </EntryBlock>
            );

            blocks.push(
                <EntryBlock key={`${section.id}-content`} type="custom" id={`${section.id}-content`} sectionId={section.id} sectionTitle={customSection.title}>
                    <div style={{ fontFamily: FONT, marginBottom: "var(--entry-space)" }} className="mb-6">
                        <ul className="list-disc pl-5 space-y-1">
                            {customSection.content.filter(Boolean).map((c, i) => (
                                <li key={i} className="text-[11.5px] text-gray-800 leading-relaxed pl-1">
                                    {c}
                                </li>
                            ))}
                        </ul>
                    </div>
                </EntryBlock>
            );
        }
    });

    return blocks;
}

export default function ExecutiveClassic({ resume }: ExecutiveClassicProps) {
    return (
        <div className="bg-white px-10 py-10 min-h-full">
            {renderExecutiveClassicBlocks(resume)}
        </div>
    );
}
